-- TheManager PostgreSQL/Aurora schema. Apply through a migration runner, never manually in production.
CREATE TABLE dim_project (project_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY, project_code VARCHAR(20) UNIQUE NOT NULL, project_name VARCHAR(100) NOT NULL, project_country VARCHAR(100), project_location VARCHAR(255), created_at TIMESTAMPTZ NOT NULL DEFAULT now());
CREATE TABLE dim_work_package (work_package_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY, work_package_code VARCHAR(20) UNIQUE NOT NULL, work_package_name VARCHAR(100) NOT NULL, wbs VARCHAR(50));
CREATE TABLE dim_equipment (equipment_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY, equipment_code VARCHAR(20) UNIQUE NOT NULL, equipment_name VARCHAR(100) NOT NULL, equipment_type VARCHAR(50), specifications TEXT);
CREATE TABLE dim_milestone (milestone_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY, milestone_number VARCHAR(20), milestone_activity VARCHAR(100) NOT NULL, milestone_description VARCHAR(255), UNIQUE(milestone_activity, milestone_description));
CREATE TABLE dim_supplier (supplier_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY, supplier_number VARCHAR(20) UNIQUE NOT NULL, supplier_name VARCHAR(100) NOT NULL, contact_name VARCHAR(100), contact_number VARCHAR(50), email_address VARCHAR(100));
CREATE TABLE dim_equipment_supplier (equipment_supplier_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY, equipment_id BIGINT NOT NULL REFERENCES dim_equipment, supplier_id BIGINT NOT NULL REFERENCES dim_supplier, unit_cost NUMERIC(18,2) NOT NULL, is_preferred BOOLEAN NOT NULL DEFAULT false, lead_time_days INTEGER, remarks TEXT, UNIQUE(equipment_id, supplier_id));
CREATE TABLE fact_purchase_order (purchase_order_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY, purchase_order_number VARCHAR(50) NOT NULL, line_item VARCHAR(20), project_id BIGINT NOT NULL REFERENCES dim_project, work_package_id BIGINT NOT NULL REFERENCES dim_work_package, supplier_id BIGINT NOT NULL REFERENCES dim_supplier, equipment_id BIGINT NOT NULL REFERENCES dim_equipment, short_text TEXT, remarks TEXT, amount NUMERIC(18,2), UNIQUE(purchase_order_number, line_item));
CREATE TABLE fact_p6_schedule (p6_schedule_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY, project_id BIGINT NOT NULL REFERENCES dim_project, work_package_id BIGINT NOT NULL REFERENCES dim_work_package, equipment_id BIGINT NOT NULL REFERENCES dim_equipment, milestone_id BIGINT NOT NULL REFERENCES dim_milestone, p6_schedule_due_date DATE NOT NULL);
CREATE TABLE fact_equipment_milestone_schedule (equipment_milestone_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY, equipment_id BIGINT NOT NULL REFERENCES dim_equipment, project_id BIGINT NOT NULL REFERENCES dim_project, work_package_id BIGINT NOT NULL REFERENCES dim_work_package, milestone_id BIGINT NOT NULL REFERENCES dim_milestone, purchase_order_id BIGINT REFERENCES fact_purchase_order, equipment_milestone_due_date DATE NOT NULL, status VARCHAR(20) NOT NULL DEFAULT 'Active');
CREATE TABLE dim_manufacturing_location (manufacturing_location_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY, equipment_id BIGINT NOT NULL REFERENCES dim_equipment, supplier_id BIGINT NOT NULL REFERENCES dim_supplier, location_address VARCHAR(255) NOT NULL);
CREATE TABLE dim_logistics_info (logistics_info_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY, equipment_id BIGINT NOT NULL REFERENCES dim_equipment, supplier_id BIGINT NOT NULL REFERENCES dim_supplier, logistics_method VARCHAR(50) NOT NULL, shipping_port VARCHAR(100), receiving_port VARCHAR(100));
CREATE TABLE fact_risk_report (report_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY, session_id VARCHAR(100) NOT NULL, conversation_id UUID NOT NULL, filename VARCHAR(255) NOT NULL, s3_key TEXT NOT NULL, report_type VARCHAR(50) NOT NULL DEFAULT 'comprehensive', created_at TIMESTAMPTZ NOT NULL DEFAULT now());

CREATE VIEW v_schedule_comparison AS
